...
class PrimeDrops(commands.Cog):
    ...
    async def check_missed_drops_run(self):
        ...
        and no recent bot message is found in the drops channel, run the drops check once.
        """
        try:
            ...
            if not channel:
                logger.error(f"Could not find drops channel with ID {DROPS_CHANNEL_ID} for missed-run check")
                return
