...
_DEFAULT_PALETTES = {
    ...
    "stats": {"bg": (25, 28, 35, 255), "accent": (74, 144, 226, 255), "text": (220, 220, 220, 255)},
}
...
class ProfileCardView(discord.ui.View):
    def __init__(
        ...
        logger.debug(f"ProfileCardView initialized with theme={theme}, layout={layout}, stats={show_stats}")
    ...
    async def toggle_layout(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            # Cycle through layouts: detailed -> compact -> stats -> detailed
            if self.layout == "detailed":
                self.layout = "compact"
            elif self.layout == "compact":
                self.layout = "stats"
            else:
                self.layout = "detailed"
    ...
    @discord.ui.button(label="Stats", style=discord.ButtonStyle.success, row=1)  # Type: Ignore
    async def toggle_stats(self, interaction: discord.Interaction, button: discord.ui.Button):
        logger.debug(f"Stats toggle requested by user {interaction.user.id}")
        try:
            if not interaction.response.is_done():  # Type: Ignore
                await interaction.response.defer(ephemeral=not self.public)  # Type: Ignore
            self.show_stats = not self.show_stats
            button.style = discord.ButtonStyle.primary if self.show_stats else discord.ButtonStyle.secondary  # Type: Ignore
            logger.info(f"User {interaction.user.id} toggled stats display: {self.show_stats}")
            await self._send_card(interaction)
        except Exception as e:
            logger.exception(f"toggle_stats failed for user {interaction.user.id}: {e}")

    @discord.ui.button(label="Badges", style=discord.ButtonStyle.secondary, row=1)  # Type: Ignore
    async def toggle_badges(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            button.style = discord.ButtonStyle.primary if self.show_badges else discord.ButtonStyle.secondary  # Type: Ignore
            logger.info(f"User {interaction.user.id} toggled badges display: {self.show_badges}")
            await self._send_card(interaction)
        except Exception as e:
            logger.exception(f"toggle_badges failed for user {interaction.user.id}: {e}")

    @discord.ui.button(label="Inventory", style=discord.ButtonStyle.secondary, row=1)  # Type: Ignore
    async def toggle_inventory(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            button.style = discord.ButtonStyle.primary if self.show_inventory else discord.ButtonStyle.secondary  # Type: Ignore
            logger.info(f"User {interaction.user.id} toggled inventory display: {self.show_inventory}")
            await self._send_card(interaction)
        except Exception as e:
            logger.exception(f"toggle_inventory failed for user {interaction.user.id}: {e}")

    @discord.ui.button(label="💾 Save Settings", style=discord.ButtonStyle.success, row=2)  # Type: Ignore
    async def save_preferences(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            settings_summary = [
                ...
                f"• Badges: **{'On' if self.show_badges else 'Off'}**",
                f"• Inventory: **{'On' if self.show_inventory else 'Off'}**"
            ]
...
async def create_profile_card(
    ...
    with PerformanceLogger(logger, f"create_profile_card layout={layout}"):
        ...
        if layout == "compact":
            ...
            base_height = 400  # Extra height for stats display

        # Add extra height if showing stats in detailed layout
        if layout == "detailed" and show_stats and user_data.get("stats"):
            base_height = 380
        ...
        font_medium = _load_font_cached(FONT_PATH, 28)
        ...
        # Nickname
        draw.text((base_x, current_y), nickname_disp, fill=text_color, font=font_large)
        current_y += 45

        # Join date
        draw.text((base_x, current_y), f"Joined: {user_data['join_date']}", fill=text_color, font=font_small)
        current_y += 30
        ...
        if show_stats and user_stats and layout != "compact":
            ...
            if layout == "stats":
                level_text = f"🎯 Level {level} | {xp_percentage:.1f}% to {level + 1}"
                draw.text((base_x, current_y), level_text, fill=accent_color, font=font_medium)
                current_y += 35
            else:
                draw.text((base_x, current_y), level_text, fill=accent_color, font=font_small)
                current_y += 25
                draw.text((base_x, current_y), f"XP: {_format_number(xp)}", fill=text_color, font=font_small)
                current_y += 25

            # Embers
            draw.text((base_x, current_y), f"Embers: {_format_number(embers)}", fill=text_color, font=font_small)
            current_y += 25

            # Message stats
            messages = user_stats.get("messages", 0)
            daily_streak = user_stats.get("daily_streak", 0)
            draw.text((base_x, current_y), f"Messages: {_format_number(messages)}", fill=text_color, font=font_small)
            current_y += 25
            draw.text((base_x, current_y), f"Streak: {daily_streak} days", fill=text_color, font=font_small)
            current_y += 25

            # Voice stats
            voice_time = user_stats.get("voice_time", 0)
            voice_sessions = user_stats.get("voice_sessions", 0)
            if voice_time > 0:
                draw.text((base_x, current_y), f"Voice: {_fmt_voice(voice_time)}", fill=text_color, font=font_small)
                current_y += 25

            # Achievements and prestige
            achievements = user_stats.get("achievements", 0)
            prestige = user_stats.get("prestige", 0)
            if achievements > 0:
                draw.text((base_x, current_y), f"Achievements: {achievements}", fill=text_color, font=font_small)
                current_y += 25
            if prestige > 0:
                draw.text((base_x, current_y), f"Prestige: {prestige}", fill=accent_color, font=font_small)
                current_y += 25
